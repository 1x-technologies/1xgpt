...
class GenieEvaluator:
    def __init__(self, checkpoint, wrapped_decode_latents, device="cuda"):
        super().__init__()
        self.model = LitWorldModel.load_from_checkpoint(checkpoint, T=WINDOW_SIZE, S=LATENT_H*LATENT_W, image_vocab_size=1001).model
        self.model.eval()
    ...
    def predict_zframe_logits(self, input_ids: torch.LongTensor) -> torch.Tensor:
        ...
        Args:
            input_ids: LongTensor of size (B, T*h*w) corresponding to flattened images in the latent space.
        ...
        for timestep in range(1, WINDOW_SIZE):
            ...
            if DEBUG:
                ...
                sample_HW, logits_CHW = self.model.maskgit_generate(
                    inputs_masked, out_t=timestep, maskgit_steps=8, temperature=0.01)
    ...
    def predict_next_frames(self, samples_THW) -> torch.Tensor:
        ...
        and decoding the latent space tokens back to the original image space.

        Args:
            samples_THW: LongTensor of size (B, T, H, W) corresponding to sampled images in the latent space.
...
def main():
    ...
    evaluator = GenieEvaluator(args.checkpoint, wrapped_decode_latents)
    ...
    for batch in tqdm(dataloader):
        ...
        # decoded_gtruth is batch size 12 for some reason ?
